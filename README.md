...
`maskValue` and `redactValue` recursively transform string values in arrays, plain objects, errors, and the own enumerable data properties of class instances and callable objects. They preserve cycles and return a copy without mutating the input. Boxed strings, including transparent proxy wrappers, become protected primitive strings; malformed boxed-string candidates are discarded rather than projected character by character. Other objects and callables become null-prototype records, so inherited methods, accessors, non-enumerable state, and custom `toJSON` methods are not retained or invoked. Transformed errors recognized by their runtime brand or non-enumerable own diagnostic data use the base `Error` prototype and protect own data properties for their name, message, stack, cause, and enumerable metadata without invoking inherited accessors.

Their `ProtectedValue<T>` return type widens transformed string literals and preserves a precise branch for unaugmented tuple items. Because TypeScript is structurally typed, it cannot prove that an array-like value is a real array. Array-shaped inputs therefore expose both the actual-array and conservative object projections; validate the runtime shape before using methods that are not shared by both branches. Other object and function types expose optional readonly non-callable data matching their normalized records. Augmented arrays retain their element type when it is recoverable, while hidden numeric tuple augmentations fall back to `unknown[]` rather than claiming an incompatible element type.
